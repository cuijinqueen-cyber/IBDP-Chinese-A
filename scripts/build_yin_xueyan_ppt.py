...
def footer(slide, page=None, total=None, light=False):
    page = len(prs.slides) if page is None else page
...
def close_read(kicker, title, quote, tags, points):
    """原文引用 + 概念标签 + 细读三点。"""
    s = prs.slides.add_slide(blank)
    bg(s)
    section_bar(s, kicker, title)
    add_round(s, Inches(0.5), Inches(1.42), Inches(12.3), Inches(1.55), WINE)
    tb(s, Inches(0.7), Inches(1.52), Inches(1.4), Inches(0.28), "原文", 11, GOLD, True)
    tb(s, Inches(0.7), Inches(1.8), Inches(11.9), Inches(1.05), quote, 14, WHITE)
    tb(s, Inches(0.55), Inches(3.08), Inches(12.2), Inches(0.32),
       "调用概念：  " + "  ·  ".join(tags), 13, CRIMSON, True)
    n = len(points)
    width = 12.3 / n - 0.12
    for i, (h, body) in enumerate(points):
        x = Inches(0.5 + i * (width + 0.12))
        add_round(s, x, Inches(3.45), Inches(width), Inches(3.1), CARD, LINE)
        add_rect(s, x, Inches(3.45), Inches(0.08), Inches(3.1), CRIMSON if i % 2 == 0 else GOLD)
        tb(s, x + Inches(0.2), Inches(3.58), Inches(width - 0.3), Inches(0.4), h, 15, CRIMSON, True)
        tb(s, x + Inches(0.2), Inches(4.05), Inches(width - 0.32), Inches(2.3), body, 13, INK2)
    footer(s)
    return s
...
tb(s, Inches(0.7), Inches(2.85), Inches(12), Inches(0.45),
   "文学概念、手法与细读", 26, RGBColor(0xF0, 0xD4, 0xC0))
tb(s, Inches(0.7), Inches(3.5), Inches(11.5), Inches(0.7),
   "先界定概念，再落到原文词句：引用 → 手法 → 效果 → 主旨。\n按「文本表层 → 艺术手法 → 象征意义 → 主题思想」展开。",
...
s = prs.slides.add_slide(blank)
bg(s)
section_bar(s, "CONTENTS", "八个板块 · 概念贯穿 · 原文细读")
items = [
    ("一", "作品定位与核心概念", "文类 · 术语表 · 细读公式"),
    ("二", "人物形象：矛盾与象征", "雪/艳 · 开篇句 · 重煞细读"),
    ("三", "人物塑造手法", "以形写神 · 白描 · 语言 · 细节"),
    ("四", "叙事手法", "插叙 · 全知 · 不可靠叙述 · 态度词"),
    ("五", "空白手法", "伊瑟尔 · 人物/环境/情节空白"),
    ("六", "环境、意象与象征", "白红 · 晚香玉 · 女祭司 · 观世音"),
    ("七", "语言特色与反讽结构", "吃红 · 四喜 · 互文 · 整体反讽"),
    ("八", "主旨与答题路径", "六层主旨 · 引用如何写进论述"),
]
...
s = prs.slides.add_slide(blank)
bg(s)
section_bar(s, "01  文学概念", "术语表（一）：谁在说？从哪看？可不可信？", "答题时先把「叙述者」和「作者」分开，否则反讽写不出来。")
terms1 = [
    ("叙述者", "讲故事的那个声音。本篇冷眼旁观，口吻倾慕、赞美尹雪艳。"),
    ("隐含作者", "文本整体流露出的价值立场。本篇冷静、批判、带悲悯。"),
    ("不可靠叙述", "布斯：叙述者规范 ≠ 隐含作者规范。裂隙即反讽。"),
    ("全知视角", "能看见所有人外貌言行，但本篇刻意不进入任何人内心。"),
    ("语调 tone", "欧阳子：嘲讽一贯，靠叙述口气传达，而非作者跳出来说。"),
    ("插叙", "主线在台北「现在」，不断插入上海往事，今昔对照。"),
]
for i, (t, b) in enumerate(terms1):
    col, row = i % 3, i // 3
    x = Inches(0.45 + col * 4.25)
    y = Inches(1.5 + row * 2.5)
    add_round(s, x, y, Inches(4.1), Inches(2.3), CARD, LINE)
    tb(s, x + Inches(0.22), y + Inches(0.18), Inches(3.65), Inches(0.4), t, 16, CRIMSON, True)
    tb(s, x + Inches(0.22), y + Inches(0.7), Inches(3.65), Inches(1.4), b, 13, INK2)
footer(s)
...
s = prs.slides.add_slide(blank)
bg(s)
section_bar(s, "01  文学概念", "术语表（二）：形象如何被做成？意义如何溢出？")
terms2 = [
    ("以形写神", "传统小说/画论：写衣着、神情、动作，让精神从外形长出。"),
    ("白描", "少堆形容词，抓最突出特征，三言两语神情毕现。"),
    ("反讽", "表面意思与真实意思分裂。分话语、情景、人物、结构四层。"),
    ("意象 / 隐喻 / 象征", "意象可感；隐喻以此喻彼；象征指向多层抽象意义。"),
    ("空白 / 未定点", "伊瑟尔：意义在阅读中填补。人物失声、态度词、环境满而人空。"),
    ("对照 / 互文", "白/红、冷/热、永远/无常。妲己褒姒等典故把她读成祸水/妖孽。"),
]
for i, (t, b) in enumerate(terms2):
    col, row = i % 3, i // 3
    x = Inches(0.45 + col * 4.25)
    y = Inches(1.5 + row * 2.5)
    add_round(s, x, y, Inches(4.1), Inches(2.3), CARD, LINE)
...
s = prs.slides.add_slide(blank)
bg(s)
section_bar(s, "01  文学概念", "细读公式：原词必须成为论点的钉子")
steps_r = [
    ("1  引", "摘关键原词，不整段抄。\n如「总也不老」「却」「算是」「吃你的红」。"),
    ("2  点", "点明概念/手法。\n这是反讽、白描、空白，还是象征？"),
    ("3  析", "分析这个词如何工作。\n谁在说？和上下文哪里打架？"),
...
for i, (t, b) in enumerate(steps_r):
    x = Inches(0.5 + i * 3.2)
    add_round(s, x, Inches(1.5), Inches(3.05), Inches(3.4), CARD, LINE)
    tb(s, x + Inches(0.2), Inches(1.7), Inches(2.65), Inches(0.55), t, 18, CRIMSON, True)
    tb(s, x + Inches(0.2), Inches(2.4), Inches(2.65), Inches(2.2), b, 14, INK2)
    if i < 3:
        tb(s, x + Inches(2.9), Inches(2.8), Inches(0.35), Inches(0.4), "→", 18, GOLD, True)
add_round(s, Inches(0.5), Inches(5.1), Inches(12.3), Inches(1.5), CARD, LINE)
tb(s, Inches(0.75), Inches(5.25), Inches(11.8), Inches(0.35), "示范句式（可直接改写进论述）", 14, CRIMSON, True)
tb(s, Inches(0.75), Inches(5.65), Inches(11.8), Inches(0.8),
   "叙述者用「永远是尹雪艳」把时间停住，这是表层赞美；但「谁能不老」的常识使这句话成为不可靠叙述的第一声。隐含作者借绝对化的「总也」，提前为全文定下反讽的调。",
   14, INK2)
footer(s)
...
close_read(
    "02  细读 · 开篇",
    "「尹雪艳总也不老」——反讽从第一句开始",
    "「尹雪艳总也不老。……不管人事怎么变迁，尹雪艳永远是尹雪艳，在台北仍旧穿着她那一身蝉翼纱的素白旗袍，一径那么浅浅的笑着，连眼角儿也不肯皱一下。」",
    ["反讽基调", "对照", "不可靠叙述", "时间象征"],
    [
        ("抓原词", "「总也」「永远」「连……也不肯」都是绝对化。叙述者把「不老」说成事实，而非比喻。"),
        ("看裂隙", "同一段里，五陵年少「头上开了顶」「两鬓添了霜」。她的恒定，靠别人的衰老衬出来。"),
        ("落主旨", "「永远」是台北人需要的幻觉。开篇已把自欺写成语法：用不容置疑的口气，说一件不可能的事。"),
    ],
)
close_read(
    "02  细读 · 风姿",
    "旋律、拍子、柳絮：均衡即非人",
    "「即使跳着快狐步，尹雪艳从来也没有失过分寸……像一球随风飘荡的柳絮，脚下没有扎根似的。尹雪艳有她自己的旋律。尹雪艳有她自己的拍子。绝不因外界的迁异，影响到她的均衡。」",
    ["以形写神", "比喻", "象征", "对照"],
    [
        ("抓原词", "「柳絮」「没有扎根」：美在轻，也在无根。上海到台北，她像风，不落地。"),
        ("看手法", "旋律/拍子/均衡是音乐隐喻。外界「迁异」，她有自己的时间，像节拍器，不像人。"),
        ("落主旨", "无根的轻盈 = 不参与历史。历史变迁是别人的；她只提供一个不变的舞台。"),
    ],
)
...
close_read(
    "02  细读 · 传说",
    "重煞：流言如何变成结构",
    "「尹雪艳的八字带着重煞，犯了白虎，沾上的人，轻者家败，重者人亡。谁知道就是为着尹雪艳享了重煞的令誉，上海洋场的男士们都对她增加了十分的兴味。……去闯闯这颗红遍了黄浦滩的煞星儿。」",
    ["不可靠叙述", "反讽", "欲望象征", "情节预示"],
    [
        ("谁在说", "先是「姊妹淘醋心」的流言。叙述者没有否认，反而写男士因此更想冒险。"),
        ("看反转", "煞气本该吓退，结果变成吸引力。禁忌刺激欲望，结构上预告王、洪、徐的结局。"),
        ("落主旨", "害他们的不是八字，是「兴味」本身。煞星是心魔的外衣。"),
    ],
)
...
close_read(
    "03  细读 · 以形写神",
    "女祭司与悲天悯人：社交场面升格为祭祀",
    "「当牌局进展激烈的当儿，尹雪艳便换上轻装，周旋在几个牌桌之间，踏着她那风一般的步子……像个通身银白的女祭司，替那些作战的人们祈祷和祭祀。」又：「叼着金嘴子的三个九，徐徐的喷着烟圈，以悲天悯人的眼光看着她这一群……狂热的互相厮杀、互相宰割。」",
    ["以形写神", "比喻", "宗教意象", "旁观视角"],
    [
        ("抓原词", "「作战」「祈祷」「祭祀」「厮杀」「宰割」：麻将被写成战争与献祭。她不下场，只巡视。"),
        ("看神情", "「徐徐」「悲天悯人」是高位的怜悯。金嘴烟、喷烟圈，把冷和倦写进动作。"),
        ("落主旨", "女祭司送人上祭坛。她悲悯的「这一群」包括自己——美人迟暮的潜台词在旁观里。"),
    ],
)
...
close_read(
    "03  细读 · 白描",
    "溃烂的眼 vs 万年青：几笔写成对照",
    "吴经理：「头发确实全白了，而且患着严重的风湿，走起路来，十分蹒跚，眼睛又害沙眼……眼圈已经开始溃烂，露出粉红的肉来。」他对她说：「阿囡，看看干爹的头发都白光喽！侬还像枝万年青一式，愈来愈年轻！」",
    ["白描", "对照", "反讽", "时间主题"],
    [
        ("抓原词", "白发、蹒跚、倒睫、溃烂、粉红肉——专写「坏掉的身体」。粉红是衰老之红，不是喜庆。"),
        ("看对照", "万年青「愈来愈年轻」是不可能的植物。他的眼在流泪，她的眼角「也不肯皱一下」。"),
        ("落主旨", "白描先把「老」钉死，后面「老当益壮」才笑得出来。事实是反讽的底板。"),
    ],
)
...
close_read(
    "03  细读 · 话语反讽",
    "「老当益壮」：夸奖把衰老说成了英勇",
    "上文写吴经理「头发确实全白了……眼圈已经开始溃烂，露出粉红的肉来」。尹雪艳却笑吟吟：「哪里的话，干爹才是老当益壮呢！」吴经理听了「心中熨帖了」，当众票了一出《坐宫》。",
    ["话语反讽", "个性化语言", "对照", "自欺"],
    [
        ("抓原词", "「老当益壮」加感叹号，语气很坚定；上文的溃烂眼睛是事实压力，词义被拧弯。"),
        ("看听话人", "吴经理需要这句麻醉。反讽不在她「说谎」，而在他甘心被假话救起。"),
        ("落主旨", "公馆贩卖的就是熨帖。语言替过时的人恢复尊严，也加速他们沉溺。"),
    ],
)
close_read(
    "03  细读 · 神谕",
    "「人无千日好」：安慰话其实是判词",
    "宋太太输了钱、感伤今昔。尹雪艳递过热毛巾：「宋家阿姊，『人无千日好，花无百日红』，谁又能保得住一辈子享荣华、受富贵呢？」文中写：这个时候，她的话「就如同神谕一般令人敬畏」。",
    ["个性化语言", "互文/谚语", "反讽", "命运主题"],
    [
        ("抓原词", "谚语本是劝解，被抬成「神谕」。她不谈宋先生外遇，只谈荣华无常。"),
        ("看功能", "熨平脾气，也把个人悲剧说成人人难免，责任被推给命运。"),
        ("落主旨", "说的是真的（谁也不能永远富贵），用的却是麻醉。真理可以帮人逃避。"),
    ],
)
...
close_read(
    "03  细读 · 细节",
    "郁金香、耳坠、樱桃：红点破了白",
    "寿宴上她「破例的在右鬓簪上一朵酒杯大血红的郁金香」。对徐壮图说「自然要跟别人不同一点」时，「发上那朵血红的郁金香颤巍巍的抖动着」。徐壮图看见「一对银耳坠子……来回地浪荡着」。席上杏仁豆腐「上面却放着两颗鲜红的樱桃」。",
    ["特征性细节", "色彩象征", "视角", "预示"],
    [
        ("抓原词", "「破例」「颤巍巍」「浪荡」「却」——白衣上的红不是装饰，是破戒与见血。"),
        ("看视角", "郁金香的抖，从旁写出她的热；耳坠的荡，从徐壮图眼里写出他已着道。"),
        ("落主旨", "「却」把不该出现的红硬嵌进白色甜食。欲望的预兆写进一颗樱桃。"),
    ],
)
...
close_read(
    "04  细读 · 态度词",
    "「算是」「还算」：叙述者替她开脱，读者听见冷",
    "「当王贵生犯上官商勾结的重罪，下狱枪毙的那一天，尹雪艳在百乐门停了一宵，算是对王贵生致了哀。」「尹雪艳离开洪处长时还算有良心，除了自己的家当外，只带走一个从上海跟来的名厨司及两个苏州娘姨。」",
    ["不可靠叙述", "空白", "反讽", "语气词"],
    [
        ("抓原词", "「停了一宵」「算是」把哀悼缩成一夜；「还算有良心」先褒后贬，良心被家当和仆人抵消。"),
        ("看谁在说", "叙述者像在帮她说话。敏感读者听见的是开脱失败——薄情被「良心」二字反衬出来。"),
        ("落主旨", "态度词是空白，也是反讽开关。她的内心始终不写，只留下这些不肯诚实的修饰。"),
    ],
)
...
close_read(
    "06  细读 · 意象",
    "晚香玉：三次甜香，一次危险",
    "「整个夏天，尹雪艳的客厅中都细细的透着一股又甜又腻的晚香玉。」徐壮图入门「就嗅中一阵沁人脑肺的甜香」。半夜离开时，「客厅中的晚香玉到了半夜，吐出一蓬蓬的浓香来」。室内本不宜置晚香玉，过浓则令人呼吸困难。",
    ["意象", "通感", "环境", "象征"],
    [
        ("抓原词", "「又甜又腻」「沁人脑肺」「吐出」——香被写成会呼吸、会吞人。腻，已经越过好闻。"),
        ("看三次", "布置（她的经营）→ 初入（他上钩）→ 夜半（他微醺，花仍在吐香）。香比人更持久。"),
        ("落主旨", "花语是危险的快乐。她如晚香玉：由纯白转为重煞，再化为丧礼的白色花圈。"),
    ],
)
close_read(
    "06  细读 · 比喻",
    "微风、精灵、观世音：神性比喻的叠用",
    "盛宴：「像一阵三月的微风，轻盈盈的闪进来……全场的人都好像给这阵风薰中了一般。」「像个冰雪化成的精灵，冷艳逼人。」送徐壮图出门：「一身白色的衣衫，双手合抱在胸前，像一尊观世音。」灵堂里「仍旧一身素白打扮」，「像一阵风一般的闪了进来」。",
    ["比喻", "以形写神", "互文", "反讽"],
    [
        ("抓原词", "风、精灵、观世音，都不是「女人」的比喻。薰中、冒出火，把迷恋写成中邪。"),
        ("看叠用", "观世音是慈悲；灵堂的风却让徐太太昏厥。同一身素白，救苦与索命并置。"),
        ("落主旨", "神性比喻越多，人味越少。象征成立，靠的是这些「不像人」的句子。"),
    ],
)
...
close_read(
    "07  细读 · 双关",
    "「四喜临门」与「吃你的红」：大喜即大凶",
    "葬礼当晚又开牌局。吴经理和了「东、南、西、北全齐了」的大四喜，喊「人家说和了大四喜，兆头不祥。我倒楣了一辈子……从此否极泰来。」尹雪艳按着他肩膀：「干爹，快打起精神多和两盘……我来吃你的红！」",
    ["情景反讽", "双关", "循环结构", "死亡主题"],
    [
        ("抓原词", "大四喜民俗上主凶。他偏说否极泰来。眼泪从「烂掉了睫毛的血红眼圈」淌下，喜里已有血。"),
        ("看双关", "「吃红」是吃彩头，也是死神吞没。新客人余经理、周董事长已经入座——下一个徐壮图。"),
        ("落主旨", "大悲接大喜，她不悲不喜。结构反讽完成：享乐逃避无常，牌一亮，命运已经到了。"),
    ],
)
close_read(
    "07  细读 · 互文",
    "妲己、褒姒、诰封：典故把幻觉写实",
    "吴家阿婆：「这种事情历史上是有的：褒姒、妲己、飞燕、太真——这起祸水！……妖孽！凡是到了乱世，这些妖孽都纷纷下凡。」另一处：作废头衔经她娇声称呼，「也如同受过诰封一般，心理上恢复了不少的优越感。」",
    ["互文", "不可靠叙述", "反讽", "怀旧主题"],
    [
        ("抓原词", "祸水名单是传统女祸叙事。阿婆的话不可全信，却暴露社会如何把男人的欲望推给女人。"),
        ("看诰封", "诰封是朝廷封赠。她用一声称呼，把作废的官衔重新加冕——幻觉有仪式。"),
        ("落主旨", "互文让她同时是妖孽与女神；诰封让公馆成为伪朝廷。台北人要的是被封过的梦。"),
    ],
)
